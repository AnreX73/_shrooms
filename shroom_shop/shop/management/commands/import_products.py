...
- Скачивает картинки по ссылкам из колонки image (через ";") и создаёт ProductImage
  ...
  ProductImage с media_type="video".
...
class Command(BaseCommand):
    ...
    def _import_row(self, row, category, kind, dry_run, skip_images):
        ...
        # уберём None для полей, где модель не допускает NULL (net_weight и т.п. допускают null=True — ок)
        defaults = {k: v for k, v in defaults.items() if v is not None or True}
        ...
        if dry_run:
            self.stdout.write(
                ...
                f"картинок: {len(image_urls)} | видео: {len(video_urls)}"
            )
            return True, False
        ...
        self.stdout.write(f"  [+] {article} — {status}")
    ...
    def _attach_media(self, product, url, media_type, order, retries=3, timeout=60):
        ...
        filename = url.rsplit("/", 1)[-1] or f"{product.article}_{order}"
        content = ContentFile(resp.content)

        image_kwargs = {"product": product, "media_type": media_type, "order": order}
        pi = ProductImage(**image_kwargs)
        if media_type == "image":
